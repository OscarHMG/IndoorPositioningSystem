...
public class BearingNorthProvider implements SensorEventListener,LocationListener{
    ...
    /**
     * Interface definition for a callback whenthe bearing changes
     */
    ...
    /**
     *Attributes for BearingNorthProvider
     *
     */
    ...
    //Arrays variables to catch the read values from the sensors
    ...
    /**
     * Minimum change of bearing(in degrees) to notify the change of the listener
     **/
     ...
     */
    public BearingNorthProvider(Context context) {
        this(context, 20, 0.5, 50);
    }
     ...
     */
    public BearingNorthProvider(Context context, int smoothing, double minDiffForEvent, int throttleTime)
    {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        boolean succes = SensorManager.getRotationMatrix(mMatrixR,mMatrixI,mValuesAccelerometer,mValuesMagneticField);

        //Now calculate the new smoothed azimuth value and saving in mAzimuth variable
        if(succes){
    ...
    public void onLocationChanged(Location location) {
        //Set the new location and update the bearing
    ...
    private void updateBearing(){
        if(!Double.isNaN(this.mAzimuth)){
            if(this.mLocation == null){
                Log.w(TAG,"Location is null. It's not true north");
                mBearing = mAzimuth;
            }else{
